...
impl EnvironmentResolver {
    ...
    pub fn resolve_with_options(&self, options: &ResolutionOptions) -> Result<IndexMap<String, String>, ResolveError> {
    ...
    fn load_source_variables(&self, source: &VariableSource) -> Result<IndexMap<String, String>, ResolveError> {
        match source {
            VariableSource::Default(vars) => Ok(vars.clone()),
            
            VariableSource::EnvFile(path) => {
                let parse_options = ParseOptions { expand_variables: false };
    ...
    fn expand_variables(
        ...
        for (key, value) in &variables {
            ...
            let expanded_value = self.expand_value(value, &variables, options, &mut expansion_stack)?;
        ...
        while let Some(start) = result.find("${") {
            if let Some(end) = result[start..].find('}') {
                ...
                if expansion_stack.contains(&var_name.to_string()) {
                    ...
                    let start_pos = expansion_stack.iter()
                ...
                let replacement = if let Some(var_value) = all_variables.get(var_name) {
                    ...
                    let expanded = self.expand_value(var_value, all_variables, options, expansion_stack)?;
                    expansion_stack.pop();
                    expanded
                } else {
...
mod tests {
    ...
    fn test_variable_expansion_basic() {
        ...
        let resolved = resolver.resolve().unwrap();
        assert_eq!(resolved.get("ENDPOINT"), Some(&"https://api.example.com/v1".to_string()));
    }
    ...
    fn test_circular_reference_detection() {
        ...
        assert!(matches!(result.unwrap_err(), ResolveError::CircularReference { .. }));
    }
}