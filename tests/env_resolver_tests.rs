...
fn test_resolve_with_variable_expansion() {
    ...
    variables.insert(
        "USERS_ENDPOINT".to_string(),
        "${API_ENDPOINT}/users".to_string(),
    );
    ...
    assert_eq!(
        resolved.get("API_ENDPOINT"),
        Some(&"https://api.example.com/v1".to_string())
    );
    assert_eq!(
        resolved.get("USERS_ENDPOINT"),
...
fn test_resolve_undefined_variable_handling() {
    ...
    variables.insert(
        "PARTIAL_REF".to_string(),
        "prefix-${UNDEFINED_VAR}-suffix".to_string(),
    );
    resolver.add_source(VariableSource::Default(variables));
...
fn test_resolve_self_reference_detection() {
    ...
    variables.insert(
        "SELF_REF".to_string(),
        "prefix-${SELF_REF}-suffix".to_string(),
    );
    resolver.add_source(VariableSource::Default(variables));
...
fn test_resolve_env_file_not_found() {
    let mut resolver = EnvironmentResolver::new();
    resolver.add_source(VariableSource::EnvFile(PathBuf::from(
...
"#,
    ...
    assert_eq!(
        resolved.get("DATABASE_HOST"),
        Some(&"prod-server".to_string())
    ); // From local env
    assert_eq!(
        resolved.get("DATABASE_NAME"),
        Some(&"myapp_production".to_string())
    ); // Expanded
    assert_eq!(
        resolved.get("API_URL"),
        Some(&"http://prod-server:3000".to_string())
    ); // Expanded
    assert_eq!(
        resolved.get("DATABASE_PASSWORD"),
