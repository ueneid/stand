...
"#;
    ...
    assert_eq!(result.get("DATABASE_URL"), Some(&"postgresql://user:pass@localhost/db".to_string()));
    assert_eq!(result.get("API_KEY"), Some(&"abc123".to_string()));
    assert_eq!(result.get("DEBUG"), Some(&"true".to_string()));
    assert_eq!(result.get("DESCRIPTION"), Some(&"A long\ndescription that spans\nmultiple lines".to_string()));
    assert_eq!(result.get("BASE_URL"), Some(&"https://api.example.com".to_string()));
    assert_eq!(result.get("FULL_URL"), Some(&"https://api.example.com/v1/users".to_string()));
}
